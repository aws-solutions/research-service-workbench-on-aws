...
import { ListDataSetAccessPermissionsRequest } from '@aws/rsw-app/lib/dataSets/listDataSetAccessPermissionsRequestParser';
...
export class DataSetService implements DataSetPlugin {
  ...
  public async removeDataSet(dataSetId: string, authenticatedUser: AuthenticatedUser): Promise<void> {
    ...
    await this._removeAuthZPermissionsForDataset(
      authenticatedUser,
      RswAuthZSubject.SWB_DATASET_ACCESS_LEVEL,
      `${dataSetId!}`,
      [projectAdmin],
      ['READ', 'UPDATE', 'DELETE']
    );
    ...
    await this._removeAuthZPermissionsForDataset(
      authenticatedUser,
      RswAuthZSubject.SWB_DATASET_UPLOAD,
      dataSetId,

...
import { RSWProjectService } from './rswProjectService';

describe('RSWProjectService', () => {
  let rswProjectService: RSWProjectService;
  ...
  beforeEach(() => {
    ...
    rswProjectService = new RSWProjectService(mockDynamicAuthZService, mockWorkbenchProjectService);
  ...
  describe('createProject', () => {
    test('create and return project', async () => {
      ...
      const project = await rswProjectService.createProject(request, mockUser);
  ...
  describe('deleteProject', () => {
    test('should call projectService and dynamicAuthorizationService', async () => {
      ...
      await rswProjectService.softDeleteProject(request, checkDependencies);
  ...
  describe('getProject', () => {
    test('should call projectService and return environment', async () => {
      ...
      // OPERATE
      const project = await rswProjectService.getProject(request);
  ...
  describe('getProjects', () => {
    test('should call projectService and return projects', async () => {
      ...
      // OPERATE
      const projects = await rswProjectService.getProjects(request);
  ...
  describe('listProjects', () => {
    test('should call projectService and dynamicAuthorizationService, and return list of projects', async () => {
      ...
      // OPERATE
      const projects = await rswProjectService.listProjects(request);
  ...
  describe('updateProject', () => {
    test('should call projectService and return project', async () => {
      ...
      // OPERATE
      const project = await rswProjectService.updateProject(request);
...
function _getPAIdentityPermissions(projectId: string): IdentityPermission[] {
  ...
  for (const action of ['CREATE']) {
    identityPermissions.push({
      ...
      subjectType: RswAuthZSubject.SWB_DATASET,
      conditions: {
  ...
  for (const action of ['READ']) {
    identityPermissions.push({
      ...
      identityType: identityType,
      subjectId: allSubjectId,
      subjectType: RswAuthZSubject.SWB_DATASET_LIST,
      conditions: {
        projectId: { $eq: projectId }
      }
    });
  ...
  for (const action of ['CREATE', 'READ']) {
    identityPermissions.push({
      ...
      identityType: identityType,
      subjectId: allSubjectId,
      subjectType: RswAuthZSubject.SWB_ENVIRONMENT,
      conditions: {
        projectId: { $eq: projectId }
      }
    });
  ...
  for (const action of ['READ']) {
    identityPermissions.push({
      ...
      identityType: identityType,
      subjectId: allSubjectId,
      subjectType: RswAuthZSubject.SWB_ENVIRONMENT_TYPE
    });
  }
  ...
  for (const action of ['READ']) {
    identityPermissions.push({
      ...
      identityType: identityType,
      subjectId: allSubjectId,
      subjectType: RswAuthZSubject.SWB_ETC,
      conditions: {
        projectId: { $eq: projectId }
      }
    });
  ...
  for (const action of ['READ']) {
    identityPermissions.push({
      ...
      identityType: identityType,
      subjectId: allSubjectId,
      subjectType: RswAuthZSubject.SWB_PROJECT_LIST
    });
  }
  // Project permissions
  for (const action of ['READ', 'UPDATE', 'DELETE']) {
    identityPermissions.push({
      ...
      identityType: identityType,
      subjectId: projectId,
      subjectType: RswAuthZSubject.SWB_PROJECT
    });
  }
  ...
  for (const action of ['CREATE', 'READ', 'DELETE']) {
    identityPermissions.push({
      ...
      identityType: identityType,
      subjectId: allSubjectId,
      subjectType: RswAuthZSubject.SWB_PROJECT_USER_ASSOCIATION,
      conditions: {
        projectId: { $eq: projectId }
      }
    });
  ...
  for (const action of ['CREATE', 'READ', 'DELETE']) {
    identityPermissions.push({
      ...
      identityType: identityType,
      subjectId: allSubjectId,
      subjectType: RswAuthZSubject.SWB_SSH_KEY,
      conditions: {
        projectId: { $eq: projectId }
      }
    });
  ...
  for (const action of ['READ']) {
    identityPermissions.push({
      ...
      identityType: identityType,
      subjectId: allSubjectId,
      subjectType: RswAuthZSubject.SWB_USER
    });
  }

...
import { ProjectPlugin } from '@aws/rsw-app';
import {
...
import { RswAuthZSubject } from '../constants';
import { getProjectAdminRole, getResearcherRole } from '../utils/roleUtils';

export class RSWProjectService implements ProjectPlugin {
  ...
  private _generatePAIdentityPermissions(projectId: string, paRole: string): IdentityPermission[] {
    return [
      ...this._generateIdentityPermissions('*', RswAuthZSubject.SWB_DATASET, ['CREATE'], paRole, {
        projectId: { $eq: projectId }
      }),
      ...this._generateIdentityPermissions('*', RswAuthZSubject.SWB_DATASET_LIST, ['READ'], paRole, {
        projectId: { $eq: projectId }
      }),
      ...this._generateIdentityPermissions('*', RswAuthZSubject.SWB_ENVIRONMENT, ['CREATE', 'READ'], paRole, {
        projectId: { $eq: projectId }
      }),
      ...this._generateIdentityPermissions('*', RswAuthZSubject.SWB_ENVIRONMENT_TYPE, ['READ'], paRole),
      ...this._generateIdentityPermissions('*', RswAuthZSubject.SWB_ETC, ['READ'], paRole, {
        projectId: { $eq: projectId }
      }),
      ...
      ...this._generateIdentityPermissions('*', RswAuthZSubject.SWB_PROJECT_LIST, ['READ'], paRole),
      // Access for GetProject and GetProjects
      ...this._generateIdentityPermissions(
        projectId,
        RswAuthZSubject.SWB_PROJECT,
        ['READ', 'UPDATE', 'DELETE'],
        paRole
      ),
      ...this._generateIdentityPermissions(
        '*',
        RswAuthZSubject.SWB_PROJECT_USER_ASSOCIATION,
        ['CREATE', 'READ', 'DELETE'],
        paRole,
        { projectId: { $eq: projectId } }
      ),
      ...
      ...this._generateIdentityPermissions('*', RswAuthZSubject.SWB_USER, ['READ'], paRole)
    ];
  }
  ...
  private _generateResearcherIdentityPermissions(
    ...
    return [
      ...this._generateIdentityPermissions('*', RswAuthZSubject.SWB_DATASET_LIST, ['READ'], researcherRole, {
        projectId: { $eq: projectId }
      }),
      ...this._generateIdentityPermissions(
        '*',
        RswAuthZSubject.SWB_ENVIRONMENT,
        ['CREATE', 'READ'],
        researcherRole,
        { projectId: { $eq: projectId } }
      ),
      ...this._generateIdentityPermissions(
        '*',
        RswAuthZSubject.SWB_ENVIRONMENT_TYPE,
        ['READ'],
        researcherRole
      ),
      ...this._generateIdentityPermissions('*', RswAuthZSubject.SWB_ETC, ['READ'], researcherRole, {
        projectId: { $eq: projectId }
      }),
      // Adding a permission for ListProjects so that as soon as a Researcher gets added to their first project
      // they can begin to call ListProjects
      ...this._generateIdentityPermissions('*', RswAuthZSubject.SWB_PROJECT_LIST, ['READ'], researcherRole),
      // Access for GetProject and GetProjects
      ...this._generateIdentityPermissions(projectId, RswAuthZSubject.SWB_PROJECT, ['READ'], researcherRole),
      ...this._generateIdentityPermissions(
        '*',
        RswAuthZSubject.SWB_SSH_KEY,
        ['CREATE', 'READ', 'DELETE'],

...
export default class AuthorizationSetup {
  ...
  public async run(): Promise<void> {
    ...
    await this.createGroupIfNotExist(itAdmin, 'IT Admin group for RSW.', adminUser);
    //Purge ITAdmin permssions
    await this._purgeGroupPermissions(itAdmin, adminUser);

    //IT Admin Permissions
    const projectPermissions = this._mapActions(itAdmin, RswAuthZSubject.SWB_PROJECT);
    const projectListPermissions = this._mapActions(itAdmin, RswAuthZSubject.SWB_PROJECT_LIST, ['READ']);
    const userPermissions = this._mapActions(itAdmin, RswAuthZSubject.SWB_USER);
    const projectToUserAssociationPermissions = this._mapActions(
      itAdmin,
      RswAuthZSubject.SWB_PROJECT_USER_ASSOCIATION,
      ['CREATE', 'READ', 'DELETE']
    );
    const environmentPermissions = this._mapActions(itAdmin, RswAuthZSubject.SWB_ENVIRONMENT, [
      'READ',
      'UPDATE',
      'DELETE'
    ]);
    const environmentTypePermissions = this._mapActions(itAdmin, RswAuthZSubject.SWB_ENVIRONMENT_TYPE);
    const environmentTypeConfigPermissions = this._mapActions(itAdmin, RswAuthZSubject.SWB_ETC);
    const environmentTypeConfigListPermissions = this._mapActions(
      itAdmin,
      RswAuthZSubject.SWB_PROJECT_LIST_BY_ETC,
      ['READ']
    );
    const datasetPermissions = this._mapActions(itAdmin, RswAuthZSubject.SWB_DATASET, ['READ']);
    const awsAccountTemplateUrlsPermissions = this._mapActions(
      itAdmin,
      RswAuthZSubject.SWB_AWS_ACCOUNT_TEMPLATE_URL,
      ['CREATE']
    );
    const costCenterPermissions = this._mapActions(itAdmin, RswAuthZSubject.SWB_COST_CENTER);
    const awsAccountPermissions = this._mapActions(itAdmin, RswAuthZSubject.SWB_AWS_ACCOUNT);

import { CreateDataSetRequestParser } from '@aws/rsw-app/lib/dataSets/createDataSetRequestParser';
...
describe('negatives tests for remove dataset from project', () => {
  ...
  test('ITAdmin should not be able to remove dataset from project', async () => {
    try {
      await adminSession.resources.datasets.dataset(dataSet1Id).disassociateFromProject(project2Id);
    } catch (e) {
      checkHttpError(
        e,
        new HttpError(403, {
  ...
  test('Researcher should not be able to remove dataset from project', async () => {
    try {
  ...
  test('Project Admin of the project who does not own the dataset should not be able to remove the dataset', async () => {
    try {
  ...
  describe('user with no project', () => {
    ...
    test('User with no project cannot remove a dataset from a project ', async () => {
      try {
        await rs1Session.resources.datasets.dataset(dataSet1Id).disassociateFromProject(project1Id);
      } catch (e) {
        checkHttpError(
          e,
          new HttpError(403, {

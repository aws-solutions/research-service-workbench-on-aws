 ...
 */
import { rswNameMaxLength, lengthValidationMessage, rswDescriptionMaxLength } from '@aws/workbench-core-base';
...
describe('update environment types', () => {
  ...
  test('fails when trying to update name surpassing length', async () => {
    try {
      await adminSession.resources.environmentTypes.environmentType(testEnvTypeId).update(
        {
          name: generateRandomAlphaNumericString(rswNameMaxLength + 1)
        },
        true
      );
    } catch (e) {
      checkHttpError(
        ...
        new HttpError(400, {
          error: 'Bad Request',
          message: `name: ${lengthValidationMessage(rswNameMaxLength)}`
        })
      );
    }
  });
  test('fails when trying to update description surpassing length', async () => {
    try {
      await adminSession.resources.environmentTypes.environmentType(testEnvTypeId).update(
        {
          description: generateRandomAlphaNumericString(rswDescriptionMaxLength + 1)
        },
        true
      );
    } catch (e) {
      checkHttpError(
        ...
        new HttpError(400, {
          error: 'Bad Request',
          message: `description: ${lengthValidationMessage(rswDescriptionMaxLength)}`
        })
      );
    }
  });

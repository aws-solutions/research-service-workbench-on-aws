...
describe('Create Project negative tests', () => {
  ...
  beforeEach(async () => {
    ...
    const { data: costCenter } = await adminSession.resources.costCenters.create({
      name: generateRandomString(10, validRswName),
      accountId: setup.getSettings().get('defaultHostingAccountId'),
      description: generateRandomString(10, validRswDescription)
    });
  ...
  describe('with a name', () => {
    describe('that belongs to an existing project', () => {
      beforeEach(async () => {
        existingProjectName = generateRandomString(10, validRswName);

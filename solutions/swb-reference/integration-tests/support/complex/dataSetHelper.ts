...
export class DataSetHelper {
    ...
    public async listAccessPoints(bucket: string, accountId: string): Promise<Array<AccessPoint>> {
        const response = await this._awsSdk.clients.s3Control.listAccessPoints({AccountId: accountId, Bucket: bucket})!;
    ...
    public async deleteS3Resources(bucket: string, dir: string): Promise<void> {
        ...
        const deleteParams: {Bucket: string, Delete: {Objects: any}} = {Bucket: bucket,Delete: { Objects: [] }};
    ...
    public async deleteDdbRecords(dataSetId: string): Promise<void> {
        await this._awsSdk.helpers.ddb.delete({pk: `DATASET#${dataSetId}`, sk: `DATASET#${dataSetId}`}).execute();

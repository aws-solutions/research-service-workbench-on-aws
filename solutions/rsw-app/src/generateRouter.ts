...
import RswAuditExtractor from './audit/rswAuditExtractor';
import RswAuditLogger from './audit/rswAuditLogger';
import RswAuditPlugin from './audit/rswAuditPlugin';
...
export function generateRouter(apiRouteConfig: ApiRouteConfig): Express {
  ...
  const writer: Writer = new RswAuditLogger();
  const rswAuditPlugin: BaseAuditPlugin = new RswAuditPlugin(writer);
  const auditService = new AuditService(rswAuditPlugin, continueOnError, requiredAuditValues, fieldsToMask);
  const excludePaths: string[] = [];
  app.use(WithAudit({ auditService, excludePaths, extractor: new RswAuditExtractor() }));
